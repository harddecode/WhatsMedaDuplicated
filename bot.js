...
const {Message, StringSession, Image, Video} = require('./whatsmeda/');
...
// Sql
const WhatsMedaDB = config.DATABASE.define('WhatsMedaDuplicated', {
    info: {
...
async function whatsMeda () {
    ...
    clh.exc_pl = exc_fn
    clh.pay = ddd
    const WhatsMedaCN = new WAConnection();
    const Session = new StringSession();
    try {
      WhatsMedaCN.version = [3, 3234, 9]
    } catch {
      console.log(`passed v${WhatsMedaCN.version}`)
    }
    WhatsMedaCN.setMaxListeners(0);
    var proxyAgent_var = ''
    if (config.PROXY.includes('https') || config.PROXY.includes('http')) {
      WhatsMedaCN.connectOptions.agent = ProxyAgent (config.PROXY)
    }
    setInterval(async () => { 
        var getGMTh = new Date().getHours()
        var getGMTm = new Date().getMinutes()
        var ann_msg = await WhatsMedaStack.daily_announcement(config.LANG)
        var ann = await WhatsMedaStack.ann()
        while (getGMTh == 19 && getGMTm == 1) {
            ...
            if (ann.video.includes('http') || ann.video.includes('https')) {
                ...
                yt.on('end', async () => {
                    return await WhatsMedaCN.sendMessage(WhatsMedaCN.user.jid,fs.readFileSync('./' + VID + '.mp4'), MessageType.video, {caption: ilan + ann_msg.replace('{user}', WhatsMedaCN.user.name).replace('{wa_version}', WhatsMedaCN.user.phone.wa_version).replace('{version}', config.VERSION).replace('{os_version}', WhatsMedaCN.user.phone.os_version).replace('{device_model}', WhatsMedaCN.user.phone.device_model).replace('{device_brand}', WhatsMedaCN.user.phone.device_manufacturer), mimetype: Mimetype.mp4});
                });
            } else {
                if (ann.image.includes('http') || ann.image.includes('https')) {
                    var imagegen = await axios.get(ann.image, { responseType: 'arraybuffer'})
                    return await WhatsMedaCN.sendMessage(WhatsMedaCN.user.jid, Buffer.from(imagegen.data), MessageType.image, { caption: ilan + ann_msg.replace('{user}', WhatsMedaCN.user.name).replace('{wa_version}', WhatsMedaCN.user.phone.wa_version).replace('{version}', config.VERSION).replace('{os_version}', WhatsMedaCN.user.phone.os_version).replace('{device_model}', WhatsMedaCN.user.phone.device_model).replace('{device_brand}', WhatsMedaCN.user.phone.device_manufacturer)})
                } else {
                    return await WhatsMedaCN.sendMessage(WhatsMedaCN.user.jid, ilan + ann_msg.replace('{user}', WhatsMedaCN.user.name).replace('{wa_version}', WhatsMedaCN.user.phone.wa_version).replace('{version}', config.VERSION).replace('{os_version}', WhatsMedaCN.user.phone.os_version).replace('{device_model}', WhatsMedaCN.user.phone.device_model).replace('{device_brand}', WhatsMedaCN.user.phone.device_manufacturer), MessageType.text)
                }
            }
        }
    }, 50000);
    ...
    setInterval(async () => { 
        if (config.AUTOBIO == 'true') {
            var timezone_bio = await WhatsMedaStack.timezone(WhatsMedaCN.user.jid)
            var date_bio = await WhatsMedaStack.datebio(config.LANG)
            const biography = '📅 ' + date_bio + '\n⌚ ' + timezone_bio
            await WhatsMedaCN.setStatus(biography)
        }
    }, 7890);
    var shs1 = ''
    var shl2 = ''
    ...
    var StrSes_Db = await WhatsMedaDB.findAll({
        where: {
          info: 'StringSession'
        }
    });
    ...
    WhatsMedaCN.logger.level = logger_levels
    var nodb;
    if (StrSes_Db.length < 1) {
        nodb = true;
        WhatsMedaCN.loadAuthInfo(Session.deCrypt(config.SESSION)); 
    } else {
        WhatsMedaCN.loadAuthInfo(Session.deCrypt(StrSes_Db[0].dataValues.value));
    }
    WhatsMedaCN.on('open', async () => {
        console.log(
            chalk.blueBright.italic('✅ Login Information Updated!')
        );
        const authInfo = WhatsMedaCN.base64EncodedAuthInfo();
        if (StrSes_Db.length < 1) {
            await WhatsMedaDB.create({ info: "StringSession", value: Session.createStringSession(authInfo) });
        } else {
            await StrSes_Db[0].update({ value: Session.createStringSession(authInfo) });
        }
    })    
    WhatsMedaCN.on('connecting', async () => {
...
${chalk.blue.italic('ℹ️ Connecting to WhatsApp... Please Wait.')}`);
    ...
    WhatsMedaCN.on('open', async () => {
        ...
        let afwhmeda = config.WORKTYPE == 'public' ? ' Public' : ' Private'
        console.log(chalk.bgGreen('🐺 WhatsMeda' + afwhmeda));
        await new Promise(r => setTimeout(r, 500));
        let EVA_ACTİON = config.LANG == 'TR' || config.LANG == 'AZ' ? '*WhatsMeda Chatbot Olarak Çalışıyor!* 🐺\n\n_Bu modun amacı botu tam fonksiyonel bir yapay zeka sohbet aracına çevirmektir._\n_Normal moda dönmek için_ *.fulleva off* _komutunu kullanabilirsiniz._\n\n*WhatsMeda Kullandığın İçin Teşekkürler 💌*\n    *- Eva*' : '*WhatsMeda Working as a Chatbot! 🐺*\n\n_The purpose of this mod is to turn the bot into a fully functional AI chatbot._\n_You can use the_ *.fulleva off* _command to return to normal mode._\n\n*Thanks For Using WhatsMeda 💌*\n    *- Eva*'
        if (WhatsMedaCN.user.jid == one || WhatsMedaCN.user.jid == two || WhatsMedaCN.user.jid == three || WhatsMedaCN.user.jid == four || WhatsMedaCN.user.jid == five || WhatsMedaCN.user.jid == six || WhatsMedaCN.user.jid == seven || WhatsMedaCN.user.jid == eight) {
            await WhatsMedaCN.sendMessage(WhatsMedaCN.user.jid,nw, MessageType.text), console.log(nw), await new Promise(r => setTimeout(r, 1000))
            await heroku.get(baseURI + '/formation').then(async (formation) => { 
                forID = formation[0].id; 
                await heroku.patch(baseURI + '/formation/' + forID, { 
                    body: { 
        ...
        if (config.FULLEVA == 'true') {
            var eva_msg = await WhatsMedaStack.eva_if(config.LANG)
            await WhatsMedaCN.sendMessage(WhatsMedaCN.user.jid, eva_msg, MessageType.text)
        }
        else {
            var af_start = await WhatsMedaStack.work_type(config.WORKTYPE, config.LANG)
            await WhatsMedaCN.sendMessage(WhatsMedaCN.user.jid, af_start, MessageType.text)
        }
        await git.fetch();
        var commits = await git.log([config.BRANCH + '..origin/' + config.BRANCH]);
        if (commits.total === 0) {
            await WhatsMedaCN.sendMessage(
                WhatsMedaCN.user.jid,
        ...
        if (msg.messageStubType === 32 || msg.messageStubType === 28) {
            ...
            if (gb !== false) {
                if (gb.message.includes('{gpp}')) {
                    var ppUrl = await WhatsMedaCN.getProfilePicture(msg.key.remoteJid) 
                    var nwjson = await WhatsMedaCN.groupMetadata(msg.key.remoteJid)
                    const resim = await axios.get(ppUrl, {responseType: 'arraybuffer'})
                    await WhatsMedaCN.sendMessage(msg.key.remoteJid, Buffer.from(resim.data), MessageType.image, { mimetype: Mimetype.png, caption: gb.message.replace('{gpp}', '').replace('{botowner}', WhatsMedaCN.user.name).replace('{gname}', nwjson.subject).replace('{gowner}', nwjson.owner).replace('{gdesc}', nwjson.desc) });
                } else {
                    var nwjson = await WhatsMedaCN.groupMetadata(msg.key.remoteJid)
                    await WhatsMedaCN.sendMessage(msg.key.remoteJid, gb.message.replace('{gname}', nwjson.subject).replace('{gowner}', nwjson.owner).replace('{gdesc}', nwjson.desc).replace('{botowner}', WhatsMedaCN.user.name), MessageType.text);
                }
            }
            return;
        ...
        events.commands.map(
            async (command) =>  {
                ...
                if ((command.on !== undefined && (command.on === 'image' || command.on === 'photo')
                    ...
                    let sendMsg = false;
                    var chat = WhatsMedaCN.chats.get(msg.key.remoteJid)
                    ...
                    if (sendMsg) {
                        if (config.SEND_READ && command.on === undefined) {
                            await WhatsMedaCN.chatRead(msg.key.remoteJid);
                        }
                        var match = text_msg.match(command.pattern);
                        if (command.on !== undefined && (command.on === 'image' || command.on === 'photo' )
                        && msg.message.imageMessage !== null) {
                            whats = new Image(WhatsMedaCN, msg);
                        } else if (command.on !== undefined && (command.on === 'video')
                        && msg.message.videoMessage !== null) {
                            whats = new Video(WhatsMedaCN, msg);
                        } else {
                            whats = new Message(WhatsMedaCN, msg);
                        }
                        ...
                        try {
                            ...
                            var error_report = await WhatsMedaStack.error(config.LANG)
                            await WhatsMedaCN.sendMessage(WhatsMedaCN.user.jid, error_report.replace('{real_error}', error), MessageType.text, {detectLinks: false})

                            if (config.LANG == 'TR' || config.LANG == 'AZ') {
                                if (error.message.includes('URL')) {
                                    return await WhatsMedaCN.sendMessage(WhatsMedaCN.user.jid, '*⚕️ HATA ÇÖZÜMLEME [WHATSMEDA] ⚕️*' + 
                                        '\n========== ```Hata Okundu!``` ==========' +
                                    ...
                                    return await WhatsMedaCN.sendMessage(WhatsMedaCN.user.jid, '*🙇🏻 Maalesef Bu Hatayı Okuyamadım! 🙇🏻*' +
                                ...
                                if (error.message.includes('URL')) {
                                    return await WhatsMedaCN.sendMessage(WhatsMedaCN.user.jid, '*⚕️ ERROR ANALYSIS [WHATSMEDA] ⚕️*' + 
                                        '\n========== ```Error Resolved!``` ==========' +
                                    ...
                                    return await WhatsMedaCN.sendMessage(WhatsMedaCN.user.jid, '*🙇🏻 Sorry, I Couldnt Read This Error! 🙇🏻*' +
